...
Public API: :func:`to_cyrillic`, :func:`to_latin`, :func:`transliterate`.
Ported from the standalone ``uzbek_transliterator`` project.
"""
...
YA_WORDS = {"po(ya)bzal": "пойабзал", "po(ya)ndoz": "пойандоз", "po(ya)fzal": "пойафзал"}
YE_WORDS = {
    "i(ye)": "ийе",
    "konve(ye)r": "конвейер",
    "ple(ye)r": "плейер",
...
def transliterate(text: str, to_variant: Direction) -> str:
    """Transliterate ``text`` to Cyrillic or Latin.

    Args:
        text: input string, Uzbek in either Latin or Cyrillic script.
        to_variant: ``"cyrillic"`` or ``"latin"`` — the target script.

    Returns:
        The transliterated string. Characters outside the Uzbek alphabet
        are passed through unchanged.

    Raises:
        ValueError: if ``to_variant`` is not ``"cyrillic"`` or ``"latin"``.
    """
    if to_variant == "cyrillic":
        return to_cyrillic(text)
    if to_variant == "latin":
        return to_latin(text)
    msg = f"invalid to_variant {to_variant!r}, expected 'cyrillic' or 'latin'"
